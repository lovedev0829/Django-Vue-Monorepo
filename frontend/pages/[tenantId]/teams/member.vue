...
<script setup>
    ...
    import PersonalMemeberComponent from '@/components/organization/create.vue';
    import OrganizationMemberComponent from '@/components/members/create.vue';
    ...
    // Initialize the menu ref with 'member' to make it the active button by default
    ...
    watch(menu, (newValue) => {
        if (newValue?.value === 'general') {
            $router.push(generateTeamPath('teams/general'));
        } else if (newValue?.value === 'subscription') {
            $router.push(generateTeamPath('subscriptions'));
        }
        // Add more conditions for other menu options if needed
    });
